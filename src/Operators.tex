\chapter{Extensions}
\label{ch:extension}
It became clear that both the language from \cite{borgwardt2015temporalizing} and consequently the implementation of the BHE \cite{boundedhistoryencodingalgorithm}
must be extended to provide relevant answers to the PTQs specified in Chapter \ref{ch:queries}.
In this chapter, the extensions are introduced one by one and examined for feasibility.
The definitions of $\mathsf{eval}^{n}(\alpha)$, $\Phi_{0}(\psi)$ and $\Phi_{i}(\psi)$ from \cite{borgwardt2015temporalizing} are modified,
upon which the modifications are proven to be correct.

\section{Operators $\Box$, $\Box^{-}$, $\Diamond$, $\Diamond^{-}$}
\label{sec:extension/extensions/operatorsnotimplemented}
First, the algorithm presented in \cite{borgwardt2015temporalizing} is extended by the operators
$\Box \phi$ (always), $\Box^{-} \phi$ (always in the past), $\Diamond \phi$ (eventually), $\Diamond^{-} \phi$ (sometime in the past).
...
\begin{definition}[semantics of temporal queries ct.\ Definition 3.3 in \cite{borgwardt2015temporalizing}]
    ...
    \begin{table}[H]
        ...
        \end{tabular*}
        \caption{Satisfaction relation of temporal queries $\Box, \Box^{-}, \Diamond$ and $\Diamond^{-}$}
        \label{tab:extension/operatorsnotimplemented/satisfactionrelation}
    \end{table}
    $\mathsf{FVar}(\phi)$ denotes the set of \textit{free variables} of a TQ and is defined as the union of the sets $\mathsf{FVar}(\psi)$ of
    all queries $\psi$ occurring in $\phi$. $\mathsf{N}_{\mathsf{C}}$ denotes a set of \textit{constants}.
    If $\mathfrak{I}, i \models \mathfrak{a}(\phi)$, then $\mathfrak{a}$ is called an \textit{answer} to $\phi$ w.r.t. $\mathfrak{I}$ at time point $i$.
...
\begin{itemize}
    \item $\Box \phi_{1}$ is equivalent to $\phi_{1}$ because $\CIRCLE \Box \phi_{1}$ is tautological and
...
\begin{restatable}[ct.\ Proposition 3.4 in \cite{borgwardt2015temporalizing}]{proposition}{propOperators}
...
\begin{proof}
    ...
    The other two cases work similarly and can be found in Appendix \ref{ch:appendixC}.
    The proof works mainly based on semantics.
    \begin{itemize}
        \item[1.] $\Box \phi_{1} \equiv \phi_{1} \wedge \CIRCLE \Box \phi_{1}$
        \begin{align}
        ...
        \end{align}
        \eqref{eq:extension/operatorsnotimplemented/proofProp2/eq1.3} is equivalent to \eqref{eq:extension/operatorsnotimplemented/proofProp2/eq1.2} because
        \begin{itemize}
        ...
        \item[4.] $\Diamond^{-} \phi_{1} \equiv \phi_{1} \vee \Circle^{-} \Diamond^{-} \phi_{1}$
        \begin{align}
        ...
        \end{align}
        \eqref{eq:extension/operatorsnotimplemented/proofProp2/eq4.3} is equivalent to \eqref{eq:extension/operatorsnotimplemented/proofProp2/eq4.2} because
        \begin{itemize}
...
For this, the notation of \textit{answer terms} is needed.
Using the same simplification as in \cite{borgwardt2015temporalizing}, the following assumes that $\mathsf{N}_{\mathsf{V}}$, the set of \textit{variables}, is finite
...
\begin{definition}[answer\ term ct.\ Definition 6.1 in \cite{borgwardt2015temporalizing}]
    ...
    The set $\mathsf{AT}^{i}_{\phi}$ of all \textit{answer terms} for $\phi$ at $i \geq 0$ is the smallest set satisfying the following conditions:
    \begin{itemize}
        \item every set $A \subseteq \Delta^{\mathsf{N}_\mathsf{V}}$ is an answer term for $\phi$ at $i$;
        \item every variable $x^{\psi}_{j} \in \mathsf{Var}^{\phi}_{j}$ with $j \leq i$ is an answer term for $\phi$ at $i$; and
...
\begin{theorem}
    ...
    The extension of the algorithm from \cite{borgwardt2015temporalizing} by $\Box, \Box^{-}, \Diamond$ and $\Diamond^{-}$ preserves correctness and boundedness.
\end{theorem}
...
\begin{proof}
    ...
    \begin{proof}
        ...
        This is by induction equal to $\mathsf{Ans}(\psi_{1}, \mathfrak{I}^{(n)},0)$ which then is, as shown in Proposition \ref{prop:extension/operatorsnotimplemented/prop2},
    ...
    \begin{restatable}[ct.\ Lemma 6.4 in \cite{borgwardt2015temporalizing}]{lemma}{lemmaOperators}
    ...
    \begin{proof}
        ...
        This is shown for two cases here.
        The other cases can be found in Appendix \ref{ch:appendixC}.

        \noindent If $\psi = \Box^{-}\psi_{1}$, then
        \begin{equation}
            \notag
    ...
    This concludes the proof of Theorem \ref{th:extension/operatorsnotimplemented/corretandbounded}.
\end{proof}


\section{Filter Operator}
\label{sec:extension/extensions/filtering}
\input{QueryFiltering.tex}

\section{Metric Temporal Operators}
\label{sec:extension/extensions/metrictemporaloperators}
\input{MetricTemporalOperators.tex}

\section{Combined Extensions}
\label{sec:extension/combiningExtensions}
In the following, only the extensions of the algorithm from \cite{borgwardt2015temporalizing} by a filter operator and metric temporal operators will be discussed,
as $\Box, \Box^{-}, \Diamond$ and $\Diamond^{-}$ can be seen as special cases of MTOs, where $p = n-i$ or $p = i$, respectively.

From the proofs of Theorem \ref{th:extension/filtering/corretandbounded} and Theorem \ref{th:extension/metrictemporaloperators/corretandbounded} it follows, that extending the algorithm from \cite{borgwardt2015temporalizing} by a filter operator and MTOs preserves correctness and boundedness.

In terms of expressiveness, only the filter operator adds new expressiveness.
This has already been hinted at in Chapter \ref{ch:queries}, when introducing the MTOs and follows from Proposition \ref{prop:extension/metrictemporaloperators/prop2} and Proposition \ref{prop:extension/metrictemporaloperators/prop3}.
...
However, the MTOs are still relevant, as they can reduce the size of queries significantly.
A query without MTOs that is equivalent to, e.g. $\Box$ with a time limit $t$, consists of at least $2*t$ operators, a $\wedge$ and a $\CIRCLE$ for each time point up to the limit.
Whereas, the corresponding query with MTOs consists of only one operator $\Box_{t}$ regardless of the size of $t$.

Lastly, it is feasible to implement the extensions as the main modification needed are case-distinctions in the implementations of $\Phi_{0}(\psi)$ and $\Phi^{0}_{i}(\psi)$ \cite{boundedhistoryencodingalgorithm}.
