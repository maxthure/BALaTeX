...
This filter operator $f[\phi]$ denotes that the filter query $f$ is applied onto the result of $\phi$.
...
However, it is known that the implementation of the bounded history encoding from \cite{borgwardt2015temporalizing} is based on SQL.
...
\begin{definition}[semantics of temporal queries with a filter operator ct.\ Definition 3.3 in \cite{borgwardt2015temporalizing}]
    ...
    \begin{table}[H]
        ...
        \end{tabular*}
        \caption{satisfaction relation of TQs with a filter operator}
        \label{tab:extension/filtering/satisfactionrelation}
    \end{table}
\end{definition}
Now the query REF QUERY2 from chapter REF CHAPTER 2 can be rewritten as follows:
\begin{align}
...
\begin{theorem}
    ...
    Extending the bounded history encoding from \cite{borgwardt2015temporalizing} by a filter operator preserves correctness and boundedness.
\end{theorem}
