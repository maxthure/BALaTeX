...
\begin{proof}
    ...
    \begin{proof}
        ...
        This is by induction equal to $Ans(\psi_{1}, \mathfrak{I}^{(n)},0)$ which then is, as shown in PROPOSITION XY,
        ...
        This is by induction equal to $Ans(\psi_{2}, \mathfrak{I}^{(n)},0)$ which then is, as shown in PROPOSITION XY,
        equal to $\mathsf{Ans}(\psi, \mathfrak{I}^{(n)},0)$.\\
        If $\psi = \Circle^{-}_{p} \psi_{1}$, $\psi = \CIRCLE^{-}_{p} \psi_{1}$, $\psi = \Box^{-}_{p} \psi_{1}$ or $\psi = \Diamond^{-}_{p} \psi_{1}$ and $p > 0$, then
        \[\(\mathsf{eval}^{n}(\Phi_{0}(\psi)) = \mathsf{eval}^{n}(\Phi_{0}(\psi_{1}))\).\]
        This is by induction equal to $\mathsf{Ans}(\psi_{1}, \mathfrak{I}^{(n)},0)$ which then is, as shown in PROPOSITION XY,
        equal to $\mathsf{Ans}(\psi, \mathfrak{I}^{(n)},0)$.\\
    ...
    This concludes the proof of Theorem \ref{th:correctandbounded1}.
\end{proof}