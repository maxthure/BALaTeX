...
\begin{itemize}
    ...
    \item "Which cars cost less than 10,000 [\dots]" indicates the SQL statement "SELECT $url$ FROM $autos$ WHERE $price$ $<$ 10000"
...
\end{align}
Notice, that this kind of more sophisticated filtering is not considered in \cite{borgwardt2015temporalizing} and therefore is also not considered in the implementation of the bounded history encoding from \cite{borgwardt2015temporalizing}.
If expressed with the available operators in the language from \cite{borgwardt2015temporalizing} the implementation of the algorithm
presented in \cite{borgwardt2015temporalizing} rewrites the query into the SQL statement
\begin{align}
...
Thus, to express this query in the language from \cite{borgwardt2015temporalizing}, it needs to be extended by a filter operator.

Furthermore, the query REF QUERY3 can be rewritten as follows:
\begin{align}
...
\end{align}
This kind of query is highly complex when expressed in the language from \cite{borgwardt2015temporalizing}.
...
Consequently, it is necessary to extend the language from \cite{borgwardt2015temporalizing} by a filter operator and by metric temporal operators
before then extending the implementation of the bounded history encoding from \cite{borgwardt2015temporalizing} by the missing operators.
...
In the following $\mathcal{Q}-\text{query } \psi$ is defined as the SQL statement "SELECT url FROM autos WHERE NOT deleted" which returns every car that is not sold.
