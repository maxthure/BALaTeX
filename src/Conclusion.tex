...
Criteria for this are the usefulness of the answers, the time to answer one query per time point and the size of the encoding.
The practical application in this thesis was an observation of an online automotive marketplace.
The investigation was performed on the basis of PTQs and RTQs.

The PTQs were defined to determine how the BHE performs for meaningful queries.
To be able to evaluate the PTQs, the BHE \cite{borgwardt2015temporalizing} had to be extended.
As the evaluation showed, the algorithm is able to provide relevant answers at each time point within a short time.
The number of stored entries was always significantly lower than with the SVTT approach.
...
Further investigation of the algorithm showed that there is an upper bound for the size of the encoding, which can be used to estimate from which time point $t$ on
the algorithm provides an encoding which is smaller than the SVTT approach.
This was confirmed by the evaluation, since for all queries for which $t\leq10$, from $t$ on the encoding was smaller than the SVTT approach.
