...
This chapter discusses in detail the \textit{discrete data stream} used as a basis for the investigation of the algorithm from \cite{borgwardt2015temporalizing}.
It explains where this data comes from, how it is retrieved, and how it can be processed.
Finally, it explains how the already existent implementation of the BHE \cite{boundedhistoryencodingalgorithm} can query the data.

The publicly accessible data, which is used as a discrete data stream, originates from the online automotive marketplace \textit{AutoScout24} \cite{autoscout}.
It provides data on many models of various brands.
The selection of collected brands and models must be narrowed down so that the data remains manageable but still delivers noticeable results.
It is assumed that it is irrelevant which exact brands and models are collected.
Later, this thesis shows that this is true.
...
To verify whether this affects the research result, the result of the BHE \cite{borgwardt2015temporalizing} that is applied to weekly collected data is compared to the result of the BHE \cite{borgwardt2015temporalizing}, when applied to daily collected data.
Collecting the data daily artificially increases the number of time points.

The data is collected by a scraper \cite{autoscoutscraper} implemented in Python, similar to the one presented in \cite{scraperautoscout}.
...
First, it is specified which aspects of an individual car the scraper \cite{autoscoutscraper} should cover.
...
Second, the scraper \cite{autoscoutscraper} searches AutoScout24 \cite{autoscout} for a specific brand and model and saves the link to each offer the search yields.
...
Third, once all links are saved, the scraper \cite{autoscoutscraper} searches every offer individually and extracts all the previously determined aspects from the HTML, again using Beautiful Soup 4 \cite{richardson2007beautiful}.
...
This final step of storing the data in an SQLite database is required because the implementation of the BHE \cite{boundedhistoryencodingalgorithm} operates on an SQLite database as well.
This enables cross-language interaction between the scraper \cite{autoscoutscraper} and the implementation \cite{boundedhistoryencodingalgorithm}.
...
To ensure the scraper \cite{autoscoutscraper} searches every offer from last week again, it scans through all previously found offers before searching for new ones.
At the same time, reviewing each offer provides a better reference to check which offers have been deleted.
If the scraper \cite{autoscoutscraper} tries to search for a deleted offer, an error message is returned.
