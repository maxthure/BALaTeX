...
or,
\begin{align}
    &\text{"SELECT * FROM ($\Circle^{-}$(SELECT $marke$, AVG($price$) AS $price1$ FROM $autos$)}  \nonumber \\
    &\text{$\wedge$ SELECT $marke$, AVG($price$) AS $price2$ FROM $autos$) WHERE $price1 < price2$".}  \nonumber
\end{align}
...
Then it can be checked whether cross-referencing of query variables or additional filtering of query results is supported and whether the language from \cite{borgwardt2015temporalizing} can be extended by metric temporal operators.
